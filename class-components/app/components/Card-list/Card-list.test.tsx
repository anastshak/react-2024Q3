...
describe('CardList component', () => {
  ...
  const getSearchParams = () => new URLSearchParams({ search: '', page: '1' });
  ...
  test('updates search params when a card is clicked', async () => {
    renderWithProviders(<CardList cards={mockCharacters} />);

    const lukeSkywalkerCard = screen.getByText('Luke Skywalker');
    await userEvent.click(lukeSkywalkerCard);

    expect(setSearchParamsMock).toHaveBeenCalledWith({ page: '1', search: '', details: '1' });
  });
});
